...
# def datos_heatmap_features(eje_x='ram', eje_y='storage'):
#     """
#     Parámetros: Dos características técnicas para los ejes
#     Retorna: Matriz para heatmap
#     """
#     valores_x = Product.objects.values_list(
#         f'features__{eje_x}', flat=True
#     ).distinct().exclude(f'features__{eje_x}'__isnull=True)
    
#     valores_y = Product.objects.values_list(
#         f'features__{eje_y}', flat=True
#     ).distinct().exclude(f'features__{eje_y}'__isnull=True)
    
#     heatmap_data = []
    
#     for y_val in valores_y:
#         row = []
#         for x_val in valores_x:
#             total = Product.objects.filter(
#                 features__{eje_x}=x_val,
#                 features__{eje_y}=y_val
#             ).aggregate(
#                 total=Sum('stock')
#             )['total'] or 0
#             row.append(total)
#         heatmap_data.append({'name': y_val, 'data': row})
    
#     return {
#         'ejex': list(valores_x),
#         'ejey': list(valores_y),
#         'datoseries': heatmap_data
#     }
